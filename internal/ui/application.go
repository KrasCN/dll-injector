...
func (app *Application) createConsolePanel() fyne.CanvasObject {
	...
	// Create console text view - directly with black background and green text
	app.consoleView = NewConsoleText(
		color.RGBA{R: 0, G: 0, B: 0, A: 255},   // Black background
		color.RGBA{R: 0, G: 240, B: 0, A: 255}, // Green text
	)
...
func (app *Application) createLeftPanel() fyne.CanvasObject {
	// DLL selection card - more compact design
	dllEntry := widget.NewEntryWithData(app.selectedDll)
	dllEntry.SetPlaceHolder("Select DLL file path...")
	// Limit input field width, leave enough space for file selection button
	dllEntry.Wrapping = fyne.TextTruncate

	browseDllButton := ActionButton(theme.FolderOpenIcon(), "", func() {
	...
	// Use fixed-size file selection button
	browseDllButton.Importance = widget.MediumImportance

	// Compact DLL selector, use relative width to avoid button being squeezed
	dllSelector := container.NewBorder(
		nil, nil, nil,
		container.NewHBox(
			layout.NewSpacer(),
			browseDllButton,
	...
	dllIcon := widget.NewIcon(theme.FileIcon())
	dllTitleLabel := widget.NewLabelWithStyle("DLL File", fyne.TextAlignLeading, fyne.TextStyle{Bold: true})

	dllTitleContainer := container.NewHBox(
		dllIcon,
		dllTitleLabel,
	)

	// More compact DLL card
	dllCard := container.NewVBox(
		dllTitleContainer,
		dllSelector,
	)
	...
	methodIcon := widget.NewIcon(theme.SettingsIcon())
	methodTitleLabel := widget.NewLabelWithStyle("Injection Method", fyne.TextAlignLeading, fyne.TextStyle{Bold: true})

	methodTitleContainer := container.NewHBox(
		methodIcon,
		methodTitleLabel,
	)
	...
	// Use vertical layout to organize title and radio button group
	methodCard := container.NewVBox(
	...
	loadingTitle := widget.NewLabelWithStyle("Loading Method", fyne.TextAlignLeading, fyne.TextStyle{Bold: true, Italic: true})
	...
	memoryTitle := widget.NewLabelWithStyle("Memory Operations", fyne.TextAlignLeading, fyne.TextStyle{Bold: true, Italic: true})
	...
	memoryCard := container.NewVBox(
		memoryTitle,
		memoryOptions,
	)

	// Advanced techniques group
	advancedTitle := widget.NewLabelWithStyle("Advanced Options", fyne.TextAlignLeading, fyne.TextStyle{Bold: true, Italic: true})
	...
	// Target process selection button
	targetProcessTitle := widget.NewLabelWithStyle("Target Process", fyne.TextAlignLeading, fyne.TextStyle{Bold: true})
	selectProcessButton := widget.NewButton("Select Process", func() {
	...
	processCard := container.NewVBox(
		targetProcessTitle,
		container.NewHBox(
			selectProcessButton,
			app.selectedProcessLabel,
		),
	)
	...
	// Make the inject button more prominent - set to larger font and double width
	injectButton.Alignment = widget.ButtonAlignCenter
	originalMinSize := injectButton.MinSize()
	injectButton.Resize(fyne.NewSize(originalMinSize.Width*2, originalMinSize.Height*1.2))
	// Button doesn't support directly setting TextStyle, use other methods to highlight

	// Inject button container - use full-width container
	injectButtonContainer := container.NewHBox(
		layout.NewSpacer(),
		injectButton,
		layout.NewSpacer(),
	)

	// Use scroll container to wrap all content, allow scrolling on small screens
...
func (app *Application) createRightPanel() fyne.CanvasObject {
	...
	// Refresh button
	refreshButton := ActionButton(theme.ViewRefreshIcon(), "", func() {
		app.refreshProcessList()
