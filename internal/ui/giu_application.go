...
// GuiApplication represents the main GUI application using giu
...
// NewGuiApplication creates a new GUI application instance
func NewGuiApplication(title string, width, height int32) *GuiApplication {
	app := &GuiApplication{
		title:       title,
		width:       width,
		height:      height,
...
func (app *GuiApplication) setupLogger() {
...
func (app *GuiApplication) addLogLine(line string) {
	app.mu.Lock()
	defer app.mu.Unlock()
...
func (app *GuiApplication) refreshProcessList() {
...
func (app *GuiApplication) Run() error {
	wnd := giu.NewMasterWindow(app.title, int(app.width), int(app.height), 0)

	// Set window icon if available
	// Note: giu doesn't have built-in icon support, but you can set it through the underlying GLFW window
...
func (app *GuiApplication) loop() {
	...
	giu.MainMenuBar().Layout(
		...
		giu.Menu("Help").Layout(
			giu.MenuItem("Help").OnClick(func() {
				app.showHelpDialog = true
			}),
	...
	// Main content
	splitRatio := float32(0.7)
	giu.SplitLayout(giu.DirectionHorizontal, &splitRatio,
		// Left panel - main controls
		app.buildLeftPanel(),
		// Right panel - process list and logs
		app.buildRightPanel(),
	)
...
func (app *GuiApplication) buildLeftPanel() giu.Widget {
...
func (app *GuiApplication) buildAntiDetectionOptions() giu.Widget {
...
func (app *GuiApplication) buildRightPanel() giu.Widget {
...
func (app *GuiApplication) buildProcessList() giu.Widget {
	app.mu.RLock()
...
func (app *GuiApplication) buildProcessTable(processes []process.ProcessEntry) giu.Widget {
...
func (app *GuiApplication) buildLogConsole() giu.Widget {
	app.mu.RLock()
	logText := app.logText
	app.mu.RUnlock()
...
func (app *GuiApplication) buildDialogs() {
	// About dialog
...
func (app *GuiApplication) onInjectClicked() {
...
func (app *GuiApplication) performInjection() {
...
func (app *GuiApplication) Log() *zap.Logger {
	return app.logger
}
